		 ...
		 request.setCharacterEncoding("utf-8");//设置字符
			   ...
			   SimpleDateFormat f=new SimpleDateFormat("yyyy/MM/dd HH:mm");
				 String str=f.format(new Date());
		          // 准备上传目录
		          String path = this.getServletContext().getRealPath("himages");
		          File fpath = new File(path);
		          if(!fpath.exists()){
		             fpath.mkdir();
		         }
		         File limg=new File(path+'/'+session.getAttribute("hname"));
		         if(limg.exists()){
		        	 limg.delete();
		         }
		         // 实例化组件
		         ...
		         try {
		             ...
		             // 提示
		             out.println("<script>alert('头像上传成功！');location.href='main.jsp';</script>");
		             ...
		             prt1=conn.prepareStatement("delete from headimg where username=?");
		             prt1.setString(1, username);
		           prt1.executeUpdate();
		             prt=conn.prepareStatement("insert into headimg(username,imgname,ptime,uid) values(?,?,?,?)");
